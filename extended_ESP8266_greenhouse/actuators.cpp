...
void writeActuator(SensorDriver driver, uint8_t pin, bool level) {
   Serial.printf("[WRITE] digital -> LEVEL: %s PIN: %d \n", level ? "high":"low", pin);
    switch (driver) {
        ...
        default:
            deviceLog("WARN", "actuator.driver",
                      "writeActuator: unsupported driver " + String((int)driver) +
                      " on pin " + String((unsigned)pin));
            break;
    }
}
void writeActuatorPWM(SensorDriver driver, uint8_t pin, float dutyPercent) {
    switch (driver) {
        ...
        default:
            deviceLog("WARN", "actuator.driver",
                      "writeActuatorPWM: unsupported driver " + String((int)driver) +
                      " on pin " + String((unsigned)pin));
            break;
    }
}